...
// HandleDeleteStatuspageDomainById docs
// @Summary Delete statuspage domain by ID
// @Description Allows a user to delete a statuspage domain with specified ID that belongs to a statuspage with given ID
...
// @Router /statuspage/{statuspageId}/domain/{domainId} [delete]
func HandleDeleteStatuspageDomainById(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if db.Unscoped().Delete(&domain).Error != nil {
			helpers.DBInteractionFailed(c)
			return
		}
