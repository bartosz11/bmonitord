...
// HandleGetLastHeartbeatForTargetId docs
...
// @Accept json
// @Produce json
// @Param id path uint true "ID of target to get the last heartbeat info of"
...
// @Router /heartbeat/{id}/last [get]
func HandleGetLastHeartbeatForTargetId(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err = db.Joins("Target").Order("heartbeats.timestamp desc").First(&heartbeat, `"Target"."id" = ? and ("Target"."public" = true or ("Target"."user_id" = ? and ?))`, targetID, user.ID, userAuthenticated).Error
